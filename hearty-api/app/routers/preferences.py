...
class UserPreferencesSchema(BaseModel):
    ...
    use_on_device_model: Literal['parakeetCtc110m', 'parakeet'] = 'parakeetCtc110m'
...
def _row_to_schema(hp: dict, np: dict) -> UserPreferencesSchema:
    ...
    return UserPreferencesSchema(
        ...
        # Coerce legacy/dropped values (e.g. 'moonshine', 'zipformerGigaspeech')
        # to the default so a stale row can't 500 the response on the Literal.
        use_on_device_model=(
            np.get("use_on_device_model")
            if np.get("use_on_device_model") in ("parakeetCtc110m", "parakeet")
            else "parakeetCtc110m"
        ),
    )

...
**Goal:** Replace API-direct providers with a local-first Drift database so the app functions fully offline, persisting logged data until it syncs to the server.

**Architecture:** Five new Drift tables serve as the source of truth for all mutable data. Providers read from local tables via reactive streams; writes go to local tables immediately. A rewritten SyncService pushes pending records and pulls server data when connected, then triggers the idle analysis worker.
...
**Modify:**
- `lib/core/offline/offline_database.dart` — new tables, schema v2 migration, test constructor
...
- `lib/core/api/providers/meals_provider.dart` — StreamNotifier, local-first writes
...
- `lib/core/api/hearty_api_client.dart` — remove offline_queue dependency from write methods
- `lib/core/sync/sync_service.dart` — full rewrite: push/pull/analysis/pruning
- `lib/main.dart` — update callbackDispatcher, remove WorkManager periodic sync task

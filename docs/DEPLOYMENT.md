...
> **Fail-closed env vars (security audit 2026-06-26):**
> - **`ALLOWED_ORIGINS`** — if unset/blank, CORS now **denies all** cross-origin requests (it no longer falls back to `*`). Must be set to the web origin(s) in any environment the dashboard talks to.
> - **`SUPABASE_WEBHOOK_SECRET`** — gates `POST /auth/on-login`, which is now **fail-closed**: if this is unset/blank the endpoint rejects every request (401). A secret is provisioned in `.env`. If you ever register the Supabase webhook/auth-hook to call this endpoint, configure it to send `Authorization: Bearer <SUPABASE_WEBHOOK_SECRET>`. (As of this writing no such hook is registered; new-user `health_profile`/`notification_preferences` rows are created lazily on first access, so the endpoint being closed affects nothing live.)

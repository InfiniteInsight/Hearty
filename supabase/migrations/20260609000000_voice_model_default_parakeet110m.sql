...
-- 1. Migrate rows holding dropped/legacy values (e.g. 'moonshine') to the new
--    default, so the tightened CHECK below can be added without violation.
...
-- 2. New default for rows created from here on.
ALTER TABLE notification_preferences
  ALTER COLUMN use_on_device_model SET DEFAULT 'parakeetCtc110m';

-- 3. Replace the CHECK with the final allowed set (Postgres auto-named the
